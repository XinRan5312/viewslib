...
/**
 * Created by qixinh on 16/7/5.
 */
public class CutomLightTextView extends TextView {
    public CutomLightTextView(Context context) {
        super(context);
        init(null);
    }

    public CutomLightTextView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(attrs);
    }

    public CutomLightTextView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public void init(AttributeSet attrs) {
        ...
        if (attrs != null) {
            TypedArray a = getContext().obtainStyledAttributes(attrs, R.styleable.LightTextView);

            String typefaceName = a.getString(R.styleable.LightTextView_typeface);
            if (typefaceName != null) {
                Typeface tf = Typeface.createFromAsset(getContext().getAssets(), String.format("fonts/%s.ttf", typefaceName));
                setTypeface(tf);
            }

            if (a.hasValue(R.styleable.LightTextView_foreground)) {
                Drawable foreground = a.getDrawable(R.styleable.LightTextView_foreground);
                if (foreground != null) {
                    this.setForegroundDrawable(foreground);
                } else {
                    this.setTextColor(a.getColor(R.styleable.LightTextView_foreground, 0xff000000));
                }
            }

            if (a.hasValue(R.styleable.LightTextView_background2)) {
                Drawable background = a.getDrawable(R.styleable.LightTextView_background2);
                if (background != null) {
                    this.setBackgroundDrawable(background);
                } else {
                    this.setBackgroundColor(a.getColor(R.styleable.LightTextView_background2, 0xff000000));
                }
            }

            if (a.hasValue(R.styleable.LightTextView_innerShadowColor)) {
                this.addInnerShadow(a.getFloat(R.styleable.LightTextView_innerShadowRadius, 0),
                        a.getFloat(R.styleable.LightTextView_innerShadowDx, 0),
                        a.getFloat(R.styleable.LightTextView_innerShadowDy, 0),
                        a.getColor(R.styleable.LightTextView_innerShadowColor, 0xff000000));
            }

            if (a.hasValue(R.styleable.LightTextView_outerShadowColor)) {
                this.addOuterShadow(a.getFloat(R.styleable.LightTextView_outerShadowRadius, 0),
                        a.getFloat(R.styleable.LightTextView_outerShadowDx, 0),
                        a.getFloat(R.styleable.LightTextView_outerShadowDy, 0),
                        a.getColor(R.styleable.LightTextView_outerShadowColor, 0xff000000));
            }

            if (a.hasValue(R.styleable.LightTextView_strokeColor)) {
                float strokeWidth = a.getFloat(R.styleable.LightTextView_strokeWidth, 1);
                int strokeColor = a.getColor(R.styleable.LightTextView_strokeColor, 0xff000000);
                float strokeMiter = a.getFloat(R.styleable.LightTextView_strokeMiter, 10);
                Paint.Join strokeJoin = null;
                switch (a.getInt(R.styleable.LightTextView_strokeJoinStyle, 0)) {
                    case (0):
