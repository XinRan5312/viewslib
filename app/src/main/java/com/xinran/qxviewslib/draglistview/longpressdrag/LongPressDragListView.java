 ...
 */
public class LongPressDragListView extends ListView {
    ...
    public static final int FLING = 0;//快速向左滑动删除item
    public static final int SLIDE_RIGHT = 1;//点击左边删除item
    public static final int SLIDE_LEFT = 2;//点击右边删除item
    ...
    public LongPressDragListView(Context context) {
        this(context, null);
    }

    public LongPressDragListView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public LongPressDragListView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (mDragListener != null || mDropListener != null) {
            switch (ev.getAction()) {
                case MotionEvent.ACTION_DOWN:
                    ...
                    if (mRemoveMode == SLIDE_RIGHT) {//点击每个Item右边80以内的距离将删除这个item
                        if (x > item.getRight() - 80) {
                            preDrag(y, ev, itemnum, item);
                            return false;
                        }
                    }
                    if (mRemoveMode == SLIDE_LEFT) {//点击每个Item左边80以内的距离将删除这个item
                        if (x < 80) {
                            preDrag(y, ev, itemnum, item);
                            return false;
                        }
                    }
                    if (x < 200&&x>item.getLeft()) {// 点击离左边距离200以内然后就可拖动
                        preDrag(y, ev, itemnum, item);
                        return false;
                    }
                    mDragView = null;
    ...
    private void preDrag(int y, MotionEvent ev, int itemnum, ViewGroup item) {
