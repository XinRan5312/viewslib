 ...
 */
public class QxCustomViewpager extends ViewPager {

    public static final String TAG = QxCustomViewpager.class.getSimpleName();
    ...
    public QxCustomViewpager(Context context) {
        this(context,null);
    }

    public QxCustomViewpager(Context context, AttributeSet attrs) {
        super(context, attrs);
        setClipChildren(false);
        // now style everything!
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.QXCustomViewPager);
        int effect = ta.getInt(R.styleable.QXCustomViewPager_style, 0);
        String[] transitions = getResources().getStringArray(R.array.qx_effects);
        setTransitionEffect(TransitionEffect.valueOf(transitions[effect]));
        setFadeEnabled(ta.getBoolean(R.styleable.QXCustomViewPager_fadeEnabled, false));
        setOutlineEnabled(ta.getBoolean(R.styleable.QXCustomViewPager_outlineEnabled, false));
        setOutlineColor(ta.getColor(R.styleable.QXCustomViewPager_outlineColor, Color.WHITE));
        switch (mEffect) {
            case Stack:
            case ZoomOut:
                setFadeEnabled(true);
